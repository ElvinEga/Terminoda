...
export function HostsView({ onConnect }: HostsViewProps) {
  ...
  return (
    <div className="flex h-full relative bg-background">
      {/* Sidebar Filter */}
      <div className="w-64 border-r border-border p-4 flex flex-col gap-6 bg-muted/20">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-input border-border rounded-lg pl-9 pr-3 py-2 text-sm text-foreground focus:outline-none focus:border-ring focus:ring-1 focus:ring-ring transition-all placeholder:text-muted-foreground"
          />
        </div>
        ...
        <div className="space-y-1">
          ...
          {groups.map((group) => {
            ...
            return (
                ...
                >
                {group}
                <span className={cn("text-xs", isActive ? "text-muted-foreground" : "text-muted-foreground group-hover:text-foreground")}>
                    {count}
                </span>
                </button>
            );
        ...
        <div className="space-y-1 mt-auto">
            ...
            {tags.length === 0 && <div className="px-2 text-xs text-muted-foreground italic">No tags found</div>}
            {tags.map(tag => (
                <div key={tag} className="flex items-center gap-2 px-2 py-1 text-sm text-muted-foreground hover:text-foreground cursor-pointer group">
      ...
      <div className="flex-1 flex flex-col bg-background">
        {/* Toolbar */}
        <div className="h-14 border-b border-border flex items-center justify-between px-6 bg-background/95">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span className="text-foreground font-medium">{selectedGroup}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <AnimatePresence mode="wait">
            {filteredHosts.length === 0 ? (
                ...
                >
                    <Icons.Server className="w-12 h-12 mb-4 text-muted-foreground/50" />
                ...
                {filteredHosts.map((host, i) => (
                    ...
                    className="group flex items-center justify-between p-3 rounded-lg border border-transparent hover:border-border hover:bg-accent transition-all cursor-pointer"
                    >
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center border border-border group-hover:border-primary transition-colors text-muted-foreground group-hover:text-foreground">
                           <Icons.Terminal className="w-5 h-5" />
                        </div>
                        <div>
                        <h4 className="text-foreground font-medium text-sm">{host.name}</h4>
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            <span className="font-mono">{host.details.host}</span>
                            <span className="text-muted-foreground">•</span>
                            <span className="text-muted-foreground">{host.details.username}</span>
                        </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            {host.tags && host.tags.map(tag => (
                                <span key={tag} className="px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-[10px] border border-border">
                                    {tag}
                                </span>
                            ))}
                            {host.group && (
                                <span className="px-2 py-0.5 rounded-full bg-accent text-muted-foreground text-[10px] border border-border">
                                    {host.group}
                                </span>
                            )}
                        </div>
                        <div onClick={(e) => e.stopPropagation()}>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className="opacity-0 group-hover:opacity-100 p-2 hover:bg-accent rounded-md transition-all text-muted-foreground hover:text-foreground">
                                        <Icons.More className="w-4 h-4" />
                                    </button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end" className="bg-popover border-border text-popover-foreground">
                                    <DropdownMenuItem onClick={() => { setEditingHost(host); setIsDialogOpen(true); }}>Edit</DropdownMenuItem>
                                    <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => setDeletingHost(host)}>Delete</DropdownMenuItem>
                                </DropdownMenuContent>
                            </DropdownMenu>
                        </div>
                    </div>
                ...
                {filteredHosts.map((host, i) => (
                    ...
                    transition={{ delay: i * 0.03 }}
                    className="group p-4 rounded-xl border border-border bg-card hover:bg-accent hover:border-primary/50 transition-all cursor-pointer flex flex-col gap-4"
                    >
                    <div className="flex items-start justify-between">
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-foreground transition-opacity p-1">
                                        <Icons.More className="w-4 h-4" />
                                    </button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end" className="bg-popover border-border text-popover-foreground">
                    ...
                    <div>
                        <h4 className="text-foreground font-medium text-sm mb-1 truncate">{host.name}</h4>
                        <p className="text-xs text-muted-foreground font-mono truncate">{host.details.username}@{host.details.host}</p>
                    </div>
                    ...
                    <div className="mt-auto pt-2 border-t border-border">
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] text-muted-foreground">{host.group || "Ungrouped"}</span>
