...
function App() {
  ...
  const handleDuplicateSession = async (sessionId: string) => {
    ...
    const tabNumber =
      sessions.filter((s) =>
        s.name.startsWith(sourceSession.name.split(" #")[0])
      ).length + 1;
    ...
    <SidebarProvider>
      <div className="flex h-screen w-full bg-background text-foreground overflow-hidden relative font-sans">
        ...
        <main className="flex-1 relative overflow-hidden bg-background flex flex-col">
          ...
          {activeNavItem !== "terminal" && (
            <div className="absolute top-14 left-0 w-full h-64 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none z-0" />
          )}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="flex-1 w-full z-10 overflow-hidden"
            >
              {renderMainContent()}
            </motion.div>
          </AnimatePresence>
